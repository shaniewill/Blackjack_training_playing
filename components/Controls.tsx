...
const Controls: React.FC<ControlsProps> = ({ onAction, canDouble, canSplit, disabled }) => {
  return (
    <div className="flex flex-wrap gap-3 sm:gap-4 justify-center w-full max-w-2xl px-4">
      <button
        onClick={() => { playSound('click'); onAction(Action.Hit); }}
        disabled={disabled}
      ...
      <button
        onClick={() => { playSound('click'); onAction(Action.Stand); }}
        disabled={disabled}
      ...
      <button
        onClick={() => { playSound('click'); onAction(Action.Double); }}
      ...
      <button
        onClick={() => { playSound('click'); onAction(Action.Split); }}
